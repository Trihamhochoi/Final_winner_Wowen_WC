...
class Wrangling_data:
    ...
    def df_wrangling(self, update_league_tb: bool = False, update_team_perf: bool = False, recal_ELO: bool = False):
        ...
        def _merge_rank(matches, rank_tb):
            # add rank of Home
        ...
        try:
            ...
            league_weight_tb = self.db.export_table_to_dataframe(table_name=tb_name)
            ...
            # Merge rank table
            # table_rank = "fifa_rank_tb"
            # rank_tb = self.db.export_table_to_dataframe(table_name=table_rank)
            # self.wrangle_df = _merge_rank(self.wrangle_df, rank_tb=rank_tb)
            #
            # # with teams are no rank => fill it with rank 190 and point 350 (last rank and smallest point)
            # self.wrangle_df = self.wrangle_df.fillna(value={'home_rank': 190,
            #                                                 'away_rank': 190,
            #                                                 'home_point': 350,
            #                                                 'away_point': 350})
            ...
            if recal_ELO:
                nation_tb = self.db.export_table_to_dataframe(table_name='nation_tb')
                self.team_dict = {i: 1500 for i in nation_tb['nation'].values}
            else:
                elo_team_rank = self.db.export_table_to_dataframe(table_name='elo_rank_tb')
            ...
            perf_team = self.db.export_table_to_dataframe(table_name=perf_name)
