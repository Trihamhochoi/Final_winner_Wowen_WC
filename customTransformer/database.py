...
class SQLiteDBManager:
    ...
    def import_dataframe_in_db(self, df, table_name):
        df.to_sql(table_name, self.conn, index=False, if_exists='replace')
        print(f"\tDataframe is import to DataBase '{self.db_name}' with name '{table_name}'")

    # Export table into dataframe
    def export_table_into_dataframe(self, table_name):
        query = f"SELECT * FROM {table_name};"
        df = pd.read_sql_query(query, self.conn)
        print(f"\tTable '{table_name}' is exported into Dataframe.")
        return df
    ...
    def update_league_tb(self, df):
        try:
            ...
            self.import_dataframe_in_db(df=league_df, table_name="league_tb")
            ...
            self.import_dataframe_in_db(df=round_df, table_name="round_tb")
            ...
            self.import_dataframe_in_db(df=league_start_table, table_name="year_start_tb")

            # Create League Weight table
            league_weight_table = pd.merge(left=tour_round_table, right=league_df, on='league_name', how='left')
            league_weight_table = pd.merge(left=league_weight_table, right=round_df, left_on='Round',
                                           right_on='round_name',
                                           how='left').drop(columns='Round')
            league_weight_table = pd.merge(left=league_weight_table, right=league_start_table, on='tour_start',
                                           how='left')
            ...
            league_weight_table['total_weight'] = league_weight_table[
                ['league_weight', 'year_weight', 'round_weight']].sum(axis=1)
            league_weight_table['updated_date'] = pd.Timestamp.now(self.tz)

            # Import to database
            self.import_dataframe_in_db(df=league_weight_table, table_name="league_weight_tb")
    ...
    #     self.import_dataframe_in_db(df=perf_team, table_name=table_name)
    #     return None

# NEW VERSION FOR TEAM PERFORMANCE
    def update_team_perf_tb_new(self, df):
        df['home_win'] = df['result'].apply(lambda x: 1 if x == 'H' else 0)
        df['away_win'] = df['result'].apply(lambda x: 1 if x == 'A' else 0)
        ...
        home_perf_squad = (home_perf
                           .groupby(['year_league_id', 'Home'], as_index=False)
                           .agg(home_perf_wm=("win_percent", wm), total_matches=('played_matches', 'sum'))
                           .rename(columns={'Home': 'Team'}))
        ...
        away_perf['win_percent'] = round(away_perf['win_matches'] / away_perf['played_matches'], 4)
        awm = lambda x: np.average(x, weights=away_perf.loc[x.index, "weight"])
        away_perf_squad = (away_perf
                           .groupby(['year_league_id', 'Away'], as_index=False)
                           .agg(away_perf_wm=("win_percent", awm), total_matches=('played_matches', 'sum'))
                           .rename(columns={'Away': 'Team'}))

        # Merge Home and Away to recalculate
        perf_team = (pd.merge(left=home_perf_squad, right=away_perf_squad, on=['year_league_id', 'Team'], how='outer',
                              suffixes=('_home', '_away'))
                     .fillna(0)
                     .sort_values(by=['year_league_id', 'total_matches_home', 'total_matches_away'],
                                  ascending=[True, False, False])
                     .reset_index(drop=True))
        perf_team['total_matches'] = (perf_team['total_matches_home'] + perf_team['total_matches_away'])
        perf_team['year_perfomance_wm'] = (perf_team['home_perf_wm'] * perf_team['total_matches_home'] + perf_team[
            'away_perf_wm'] * perf_team['total_matches_away']) / perf_team['total_matches']
...
if __name__ == '__main__':
    ...
    tb_name = "league_tb"
    lg_df = database.export_table_into_dataframe(table_name=tb_name)
